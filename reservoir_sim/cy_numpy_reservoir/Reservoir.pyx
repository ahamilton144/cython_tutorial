...
### Reservoir class - cy_numpy_reservoir
...
cdef class Reservoir():
  ...
  cdef double[:] step(self, double t, double upstream_release):
    cdef double inflow, min_flow, demand, release, delivery, x
    cdef int i
    cdef double[:] output
    ...
    output = np.array((inflow, upstream_release, min_flow, demand, release, delivery, self.storage))
